...
# Define dataset normalization values (ImageNet by default, modify if needed)
...
Section('model', 'model details').params(
    ...
    num_classes=Param(int, 'Number of classes in the dataset', default=1000)
)

Section('resolution', 'resolution scheduling').params(
    min_res=Param(int, 'the minimum (starting) resolution', default=160),
    max_res=Param(int, 'the maximum (starting) resolution', default=192),
    end_ramp=Param(int, 'when to stop interpolating resolution', default=34),
    start_ramp=Param(int, 'when to start interpolating resolution', default=29)
)
...
Section('lr', 'lr scheduling').params(
    ...
    lr=Param(float, 'learning rate', default=0.2125),
...
Section('training', 'training hyper param stuff').params(
    ...
    epochs=Param(int, 'number of epochs', default=40),
...
Section('dist', 'distributed training options').params(
    ...
    port=Param(int, 'port', default=12355)  # Fixed: Changed to int type
)
...
class CustomTrainer:
    ...
    def __init__(self, gpu, distributed, efficient, mrl, nesting_start, fixed_feature, num_classes):
    ...
    def setup_distributed(self, address, port, world_size):
        ...
        os.environ['MASTER_PORT'] = str(port)  # Convert to string for env var
    ...
    def create_optimizer(self, momentum, optimizer, weight_decay,
        ...
        # Set loss function based on nesting
        if self.nesting:
    ...
    def create_train_loader(self, train_dataset, num_workers, batch_size,
        ...
        image_pipeline: List[Operation] = [
            ...
            ToTorchImage(),
            NormalizeImage(DATASET_MEAN, DATASET_STD, np.float16)
        ]
    ...
    def create_model_and_scaler(self, arch, pretrained, distributed, use_blurpool):
        ...
        # Adjust the final classifier layer for our number of classes
        if self.nesting:
            ...
            # Replace the last layer for custom number of classes
            in_features = model.fc.in_features
            model.fc = ch.nn.Linear(in_features, self.num_classes)
        ...
        with ch.no_grad():
            with autocast():
                for images, target in tqdm(self.val_loader):
                    ...
                    if lr_tta:
                        output += torch.stack(self.model(ch.flip(images, dims=[3])), dim=0)

                    # Logging the accuracies top1/5 for each nesting level
                    for i in range(len(self.nesting_list)):
                        s = f"top_1_{self.nesting_list[i]}"
                        self.val_meters[s](output[i], target)
                        s = f"top_5_{self.nesting_list[i]}"
                        self.val_meters[s](output[i], target)
    ...
    def initialize_logger(self, folder):
        if self.nesting:
            ...
            for i in self.nesting_list:
                self.val_meters[f'top_1_{i}'] = torchmetrics.Accuracy(compute_on_step=False, task="multiclass", num_classes=1000).to(self.gpu)
                self.val_meters[f'top_5_{i}'] = torchmetrics.Accuracy(compute_on_step=False, task="multiclass", top_k=5, num_classes=1000).to(self.gpu)
            
            self.val_meters['loss'] = MeanScalarMetric(compute_on_step=False).to(self.gpu)
        else:   
            self.val_meters = {
                'top_1': torchmetrics.Accuracy(compute_on_step=False, task="multiclass", num_classes=1000).to(self.gpu),
                'top_5': torchmetrics.Accuracy(compute_on_step=False, task="multiclass", top_k=5, num_classes=1000).to(self.gpu),
    ...
    def exec(cls, gpu, distributed, eval_only, path=None):
        trainer = cls(gpu=gpu)
        if eval_only:
            print("Loading Model....."); ckpt = ch.load(path, map_location=f"cuda:{gpu}")
...
def make_config(quiet=False):
    ...
    parser = ArgumentParser(description='Custom dataset training with MRL')
...
if __name__ == "__main__":
    make_config()
    CustomTrainer.launch_from_args()